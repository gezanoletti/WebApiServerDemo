...
{
    public abstract class WebApiServerDemoTestBase : AbpIntegratedTestBase<WebApiServerDemoTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected WebApiServerDemoTestBase()
        {
        ...
        protected void UsingDbContext(Action<WebApiServerDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<WebApiServerDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<WebApiServerDemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<WebApiServerDemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<WebApiServerDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WebApiServerDemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<WebApiServerDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WebApiServerDemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<WebApiServerDemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<WebApiServerDemoDbContext, Task<T>> func)
        {
            T result;
